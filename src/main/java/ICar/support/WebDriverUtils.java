...
public class WebDriverUtils {
	...
	public static WebDriver getDriver() throws Exception {
		try {
			...
			String reuseSession = Reporter.getCurrentTestResult().getTestContext().getCurrentXmlTest().getParameter("ReuseBrowserSession");
			reuse = reuseSession != null && reuseSession.equalsIgnoreCase("true") ? true : false;
			WebDriver driver= getDriver(reuse, "");
...
//			browserVersion= remoteWebDriver.getCapabilities().getVersion();
	...
	public static WebDriver getDriver(boolean reuseSession, String windowSize) throws Exception {
		...
		try {
			...
			{
				USERNAME = test.getParameter("SauceLabUserName");
				ACCESS_KEY = test.getParameter("SauceLabKey");
				SERVER = test.getParameter("SauceLabServer");
			}
			else if(environment.equalsIgnoreCase("browserstack"))
			{
				USERNAME = test.getParameter("BrowserStackUserName");
				ACCESS_KEY = test.getParameter("BrowserStackKey");
				SERVER = test.getParameter("BrowserStackServer");
			}
			...
			if(environment.equalsIgnoreCase("saucelab") || environment.equalsIgnoreCase("browserstack"))
			{
				hubURL = new URL("https://" + USERNAME + ":" + ACCESS_KEY + "@" + SERVER + "/wd/hub");
				...
				case "CHROME" : {
					...
					if(environment.equalsIgnoreCase("browserstack"))
					{
						...
						caps.setCapability("project", "M-Files");
						caps.setCapability("build", "LoginBuild");
					...
					else
					{
						...
						caps.setCapability("maxDuration", 3600);//Maximum Test Duration
						caps.setCapability("idleTimeout", 180);//Idle Test Timeout
						caps.setCapability("name", methodName+":"+driverType.toUpperCase());
					}
...
//					capabilities.setCapability("name", methodName+":"+driverType.toUpperCase());
...
//					 opt.addArguments("--disable-bundled-ppapi-flash");
//					 opt.addArguments("--always-authorize-plugins=true");
//					 opt.addArguments("--disable-extensions"); opt.addArguments("--lang=en-US");
//					 opt.addArguments("disable-infobars"); opt.addArguments("--start-maximized");
//					
//					if (!windowSize.isEmpty())
//						opt.addArguments("--window-size=" + windowSize);
//
//					/*if(test.getParameter("headLess").toLowerCase().equalsIgnoreCase("true"))
//					{
...
//
//					DesiredCapabilities caps = DesiredCapabilities.safari();
//					//caps.setCapability("technologyPreview", true);
//					driver = new RemoteWebDriver(hubURL,caps);
//					driver.manage().deleteAllCookies();//delete all cookies
					break;
...
//						DesiredCapabilities capabilities = DesiredCapabilities.firefox();
						...
						// In Firefox, download dialog will be displayed by default but we don't want to get the dialog
						// because Selenium can't access it. Here each mime type has to be defined when the dialog should not
						// be opened.
						String mimeTypes = "text/plain,"; // .txt
						mimeTypes += "application/pdf,"; // .doc
						...
						mimeTypes += "application/octet-stream"; // For example, .dwg
						// The download dialog will not be displayed for these mime types.
...
//							driver = new RemoteWebDriver(hubURL, capabilities);
			...
			if (e.getCause().getMessage().toLowerCase().contains("error forwarding the new session") || e.getMessage().toLowerCase().contains("sessionnotcreatedexception") || e.getMessage().toLowerCase().contains("unable to create new remote session"))
	...
	public static WebDriver getDriver(boolean reuseSession, String windowSize, String driverType) throws Exception {
		...
		try {
			...
			String driverVersion = test.getParameter("driverVersion");
			String driverPort = test.getParameter("devicePort");
			String zap_SESSION_PORT = test.getParameter("zap_SESSION_PORT");
...
//				driver.manage().deleteAllCookies();//delete all cookies
			...
			if (driverType.equalsIgnoreCase("zap")) {
				...
				zapFFProxy.setHttpProxy("localhost:"+zap_SESSION_PORT)
				.setFtpProxy("localhost:"+zap_SESSION_PORT)
				.setSslProxy("localhost:"+zap_SESSION_PORT);
				//zapFFProxy.setSocksProxy("localhost:8095").setSocksVersion(socksVersion);

				//zapFFProxy.setNoProxy("localhost");

//				DesiredCapabilities cap = DesiredCapabilities.firefox();
//				FirefoxProfile fp = new FirefoxProfile();
//				fp.setPreference("network.proxy.http", "localhost");
//				fp.setPreference("network.proxy.http_port", zap_SESSION_PORT);
//				fp.setPreference("network.proxy.no_proxies_on", "localhost, 127.0.0.1"); // INSTEAD use this one
//				//fp.setEnableNativeEvents(true);
//				//fp.setPreference("app.update.auto", false);
//				//fp.setPreference("app.update.enabled", false);
//				//fp.setPreference("app.update.silent", false);
...
//				driver = new RemoteWebDriver(hubURL, DesiredCapabilities.edge());
	...
	public final static String getTestSessionNodeIP(final WebDriver driver) throws Exception {
		...
		try{
			...
			BasicHttpEntityEnclosingRequest r = new BasicHttpEntityEnclosingRequest("GET", testSessionApi.toExternalForm());
