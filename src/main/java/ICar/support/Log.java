...
public class Log {
	...
	static {
		...
		reportPortal = (xmlParameters.getParameter("reportPortal") != null) ? xmlParameters.getParameter("reportPortal")
				: "No";
	...
	public static String takeScreenShot(WebDriver driver) {
		...
		inputFile = Reporter.getCurrentTestResult().getName() + "_" + System.nanoTime() + ".png";
...
//		if (propertiesReader.runLambdaTestFromLocal())
//			WebDriverFactory.lambdaTestStatusUpdate(driver);
	...
	public static void endTestCase() {
		lsLog4j().info("****             " + "---Test Execution Ends---" + "             *****");
		Reporter.log(TEST_COND_HTML_BEGIN + "---Test Execution Ends---" + TEST_COND_HTML_END);
		ExtentReporter.endTest("---Test Execution Ends---");
	}
	...
	public static void message(String description, WebDriver driver, Boolean... takeScreenShot) {
		...
		//
		if (propertiesReader.isTakeScreenShot() != null && propertiesReader.isTakeScreenShot() == false) {
			finalDecision = false;
		}
		if (finalDecision) {
	...
	public static void pass(String description, WebDriver driver) {
		String inputFile = "";
		if (propertiesReader.isTakeScreenShot() != null && propertiesReader.isTakeScreenShot() == true) {
	...
	/**
	 * addSauceJobUrlToReport, this method will add the sauce lab job URL( video,
	 * selenium log, commands and meta data details)
	 *
	 * @param driver    - web driver object
	 *
	 * @param sauceLink - corresponding test's sauce lab job URL
	 */
	...
	/**
	 * addSauceJobUrlToReport, this method will add the lambdatest job URL( video,
	 * selenium log, commands and meta data details)
	 *
	 * @param driver    - web driver object
	 *
	...
	public static void addLambdaJobUrlToReport(WebDriver driver, String LambdaLink) {
		...
		params[0] = (params[0] == null || !params[0].toString().contains("LambdaTest link"))
				...
				: params[0] + "<br>LambdaTest link: <a href='" + LambdaLink + "' target='_blank'>" + LambdaLink
						+ "</a>";
		Reporter.getCurrentTestResult().setParameters(params);
		ExtentReporter
				.addAttribute("LambdaTest link: <a href='" + LambdaLink + "' target='_blank'>" + LambdaLink + "</a>");
	}
