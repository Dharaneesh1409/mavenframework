...
function setDefaultValues(input) {
            ...
            input.nodes.selector = 'a,button,input,select,textarea,span,p,div,td' ;
    ...
    function setDefinitions(input,pgsrc) {
        ...
        for (var i = 0, j = nodes.length; i < j; i++) {
			...
			if (node.nodeName == "TD" || node.nodeName == "SPAN" || node.nodeName == "DIV" || node.nodeName == "P"){
	...
	function rmvDuplctAtribVal(input){       //Selva
		...
		for ( var vc = 0 ; vc < lclDefLen-3; vc++ )
		{
			...
			//console.log("iteration no1 "+aval1 +" no2 " +aval2);
			if (aval1 == aval2){
